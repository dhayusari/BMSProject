...
class SpinBox(QWidget):
    ...
    def set_voltage(self):
        #Changes the voltage data based on text input
    ...
    def update_voltage(self, cell_num, voltage):
        #Updates the UI whenever the data changes from the STM32
        if cell_num == self.num:
            self.spin_box.setValue(voltage)
            self.en_button.setChecked(True)

class TempModule(QWidget):
    #SpinBOx for temperature values
    ...
    def update_temps(self, temp_num, temp):
        #Updates the UI whenever the temp is changed internally
...
class Relay(QWidget):
    #Row for relay UI, consists of which relay and on/off control
    ...
    def precharge_routine(self, checked):
        #Sends routine for precharge to microcontroller
        if checked:
            self.controller.send_data("Precharge:1")
        
    
    def update_precharge(self, state):
        #Updates the precharge. Not used currently
        if not state:
